...
func validMove(prev string, current string) bool {
	if prev == "L" {
		if current != "7" && current != "-" && current != "J" && current != "F" {
			return false
		}
	} else if prev == "J" {
		if current != "J" && current != "F" && current != "-" {
			return false
		}
	} else if prev == "7" {
		...
		if current != "F" && current != "L" && current != "J" && current != "7" && current != "-" {
			return false
		}
	} else if prev == "|" {
		if current != "|" && current != "L" && current != "J" && current != "7" && current != "-" {
			return false
		}
	} else if prev == "-" {
		if current != "-" && current != "F" && current != "L" && current != "J" && current != "7" {
			return false
		}
	}
	return true
...
func(m Map) FollowPath(x int, y int, px int, py int, length int) int {
	...
	if x < 0 || y < 0 || x >= len(m.Cord) || y >= len(m.Cord[0]) || m.Cord[y][x].char == "." {
		return -1
	}
	//if !validMove(m.Cord[py][px].char, m.Cord[y][x].char) {
	//	return -1
	//}
	...
	case "L":
		if py != y {
			return m.FollowPath(x+1, y, x, y, length+1)
		} else {
			return m.FollowPath(x, y-1, x, y, length+1)
		}
	case "|":
		if py < y {
			return m.FollowPath(x, y+1, x, y, length+1)
...
func main() {
	...
	width := len(rows[0])
	var rough Map
	rough.Cord = make([][]position, length)
	rough.Started = false
	for i, val := range rows {
		rough.Cord[i] = make([]position, width)
		for j, char := range val {
			if string(char) == "S" {
				rough.StartX, rough.StartY = j, i
				rough.X, rough.Y = j, i
			}
			rough.Cord[i][j] = position{char: string(char)}
		}
	}
	...
	fmt.Print("S")
	var a,b,c,d int
	rightChar := rough.Cord[rough.StartY][rough.StartX+1].char
	if  rightChar == "J" || rightChar == "7" || rightChar == "-" {

		a = rough.FollowPath(rough.StartX+1, rough.StartY, rough.StartX, rough.StartY, 1)
	...
	//fmt.Println(rough.StartX, rough.StartY, 0)
	if rough.StartX-1 > 0 {
		leftChar := rough.Cord[rough.StartY][rough.StartX-1].char
		if leftChar == "F" || leftChar == "L" || leftChar == "-" {

			b = rough.FollowPath(rough.StartX-1, rough.StartY, rough.StartX, rough.StartY, 1)
		}
	}
	...
	fmt.Print("S")
	//fmt.Println(rough.StartX, rough.StartY, 0)
	downChar :=  rough.Cord[rough.StartY+1][rough.StartX].char
	if downChar == "J" || downChar == "L" || downChar == "|" {
		c = rough.FollowPath(rough.StartX, rough.StartY+1, rough.StartX, rough.StartY, 1)
	}
	...
	//fmt.Println(rough.StartX, rough.StartY, 0)
	if rough.StartY-1 > 0 {
	upChar := rough.Cord[rough.StartY-1][rough.StartX].char
		if upChar == "7" || upChar == "F" || upChar == "|" {

			d = rough.FollowPath(rough.StartX, rough.StartY-1, rough.StartX, rough.StartY, 1)
		}
	}
	fmt.Println()
	fmt.Println(a, b, c, d)
